...
import { Users, DoorOpen, Tag, Boxes } from "lucide-react";

export const Route = createFileRoute("/admin/")({ component: AdminHome });

function AdminHome() {
  const [stats, setStats] = useState({ users: 0, rooms: 0, tags: 0, items: 0, presentNow: 0 });
  const [recent, setRecent] = useState<any[]>([]);

  useEffect(() => {
    (async () => {
      const [u, r, t, i, logs] = await Promise.all([
      ...
      setStats({ users: u.count ?? 0, rooms: r.count ?? 0, tags: t.count ?? 0, items: i.count ?? 0, presentNow: 0 });
      setRecent(logs.data ?? []);
    })();
  }, []);
    ...
    <RequireAdmin>
      <AppShell mode="admin">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Stat icon={Boxes} label="Itens em estoque" value={stats.items} />
        </div>
